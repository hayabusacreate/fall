...
public class easing3D : MonoBehaviour
{
    ...
    public static Vector3 SineOut(float t, float totaltime, Vector3 min, Vector3 max)
    {
        max -= min;
    ...
    public static Vector3 SineInOut3D(float t, float totaltime, Vector3 min, Vector3 max)
    {
        max -= min;
        return -max / 2 * (Mathf.Cos(t * Mathf.PI / totaltime) - 1) + min;
    }
